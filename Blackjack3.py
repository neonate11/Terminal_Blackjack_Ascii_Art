...
def yes_no_question(*args):   #the second and third input were the third and fourth input for the draw_entire_game function (you can delete this line once you fix that)
    error_spacing = ''
    error_message = ''
    while True:
    ...
    if nate_hand.calculate_value() != 21: #Don't Deal Cards if Dealer already got Blackjack
        ...
        if player_bank >= bet_per_hand:
            ...
            while decision == '':
                ...
                decision = yes_no_question('Would you like to double down on any of your hands?')
            if decision == 'y':
                for i, hand in enumerate(all_player_hands):
                    if hand.calculate_value() < 21 and player_bank >= bet_per_hand:
                        ...
                        while decision =='':
                            draw_entire_game(all_player_hands,bet_per_hand,1,i,0)
                            decision = yes_no_question(f"Would you like to double down on your{ordinals[i]} hand?")
                        if decision == 'y':
                            player_bank -= bet_per_hand
                            hand.doubleddown()
                            hand.deal_card(deck)
        ...
        for i, hand in enumerate(all_player_hands):
            ...
            while hand.calculate_value() < 21 and hand.if_doubledown() == False and Stand == False and len(hand.cards)<5:
                if len(all_player_hands) == 1: #Say this if they only have one hand
                    ...
                    while decision == '':
                        draw_entire_game(all_player_hands,bet_per_hand,1,'n',0)
                        decision = yes_no_question(f'Would you like to hit?')
                else:                          #Say this if they have multiple hands
                    decision = ''
                    while decision == '':
                        draw_entire_game(all_player_hands,bet_per_hand,1,i,0)
                        decision = yes_no_question(f'Would you like to hit on your{ordinals[i]} hand?')
                if decision == 'y':
                    hand.deal_card(deck)
...
#Determine Game Outcome, see if player can/wants to play again
    ...
    if player_bank < 10:
        ...
        go_for_highscore = ''
        while go_for_highscore == '':
            draw_entire_game(all_player_hands,bet_per_hand,0,'n',1)
            go_for_highscore = yes_no_question(dealer_outcome,result,'You Bankrupt Nate!','You and Ralph are planning a trip to Spain','Would you like to keep playing for a highscore?')
        ...
        while decision == '':
            draw_entire_game(all_player_hands,bet_per_hand,0,'n',1)
            decision = yes_no_question(dealer_outcome,result,f'Continue playing?')
        if decision == 'n':
            clear_terminal()
            chip_total = f'Your maximum chip total was ${most_money}'
            print(top_space)
